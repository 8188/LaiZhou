...
#batch_sizefor i in range(valid_start, valid_start + int((test_end - valid_start) / 16)): 
def mscred_evaluate(
	anomaly_coordinates=None, label='anomaly_score', plot=False,
	thred_b=Constant.MSCRED_ERROR_THRESHOLD, alpha=Constant.MSCRED_MODIFICATION_FACTOR, gap_time=Constant.MSCRED_TIME_GAP, 
	valid_start_point=8000, valid_end_point=10000,
	test_start_point=10000, test_end_point=20000, 
	...
	for i in range(valid_start, test_end):	
		...
		#(5, 3, 30, 30) 第5个时间步，即现在时刻，第二个[0]是第一个（最小）窗口
		...
		#print(select_matrix_error)
		#print(num_broken)
		#batch_sizeif i < valid_start + int((valid_end - valid_start) / 16): 
		if i < valid_end:
	...
	if plot:
		...
		plt.plot(test_anomaly_score, color = 'blue', alpha=0.5, linewidth = 1)
		threshold = np.full((test_num), valid_anomaly_max * alpha)
		axes.plot(threshold, color = 'r', linestyle = '--',linewidth = 2)
		if anomaly_coordinates:
			axes.scatter(anomaly_coordinates, test_anomaly_score[anomaly_coordinates], c='g', s=1)
		'''
		...
		plt.xlabel('测试时间', fontsize = 25)
		plt.ylabel('异常分数', fontsize = 25)
		...
		if not os.path.exists('./outputs'):
			os.makedirs('./outputs')
		plt.savefig(f'./outputs/{label}.jpg')
		plt.show()

	return test_anomaly_score, valid_anomaly_max * alpha
